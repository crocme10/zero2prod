...
pub async fn login(
    ...
    // Need to put shared information (between front and back) in zero2prod common.
    // Then add a dependency in backend on common
    // Then add a dependency in frontend on common
    // Then add these data structures here.
    ...
    Ok::<axum::Json<LoginResp>, Error>(Json(()))
}
